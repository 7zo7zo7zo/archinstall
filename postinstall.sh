...
#sudo pacman --noconfirm --needed -S xcolor xorg-xfd xorg-bdftopcf zathura-pdf-poppler ttf-font-awesome intellij-idea-community-edition alsa-utils xf86-input-synaptics nvidia nvidia-dkms ttf-liberation pamixer lxappearance hplip openjdk11-src
...
mkdir ~/.config
mv temp/* ~/.config
#git clone https://github.com/phillbush/xclickroot.git temp/xclickroot ; cd temp/xclickroot ; sudo make clean install
